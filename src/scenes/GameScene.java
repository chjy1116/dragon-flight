...
public class GameScene extends GameCanvas {
	...
	public void keyReleased(KeyEvent e) {
		...
		case KeyEvent.VK_2:
			...
			for(int i=0;i<enemies.size();i++)
				System.out.println("new enemy y = "+((Enemy)enemies.elementAt(i)).y );
			break;
		case KeyEvent.VK_3:
			if(isPause){
				...
				for(int i=0;i<enemies.size();i++){
					...
					System.out.println(String.format("Enemy : %d, y=%d, speed=%d", i, _buff.y, _buff.speed) );
				}
			}
			break;
	...
	void processEnemy() {
			...
			for (int i = 0; i < 5; i++) {
				...
				if (_level >= 5) {
					...
					if (1 <= _level && _level <= 3 && neuroiLevel[i] == 1
						...
						_enemy = new Enemy(neuroi[_level + 2], new Rectangle(33, 6, 76, 81), 
								i * 96 - 23, -80,	10 * (_level + 2), 6 + _speed);
					else
						_enemy = new Enemy(neuroi[_level + neuroiLevel[i]],new Rectangle(33, 6, 76, 81),
								i * 96 - 23, -80,10 * (_level + neuroiLevel[i]), 6 + _speed);
					enemies.add(_enemy);
				}
			}
			regen++;
		...
		for (int i = 0; i < enemies.size(); i++) {
			_buff = (Enemy) enemies.elementAt(i);
			_buff.move();
			boolean ret = _buff.process(myX, myY, new Rectangle(12, 20, 55, 50));
